...
class ConfigBase(context: Context) : BaseConfig(context) {
    companion object {
        fun newInstance(context: Context) = ConfigBase(context)
    }
    ...
    var bassStrength: Int
        get() = prefs.getInt(BASS_STRENGTH, 0)
        set(bassStrength) = prefs.edit().putInt(BASS_STRENGTH, bassStrength).apply()

    var virtualizerStrength: Int
        get() = prefs.getInt(VIRTUALIZER_STRENGTH, 0)
        set(virtualizerStrength) = prefs.edit().putInt(VIRTUALIZER_STRENGTH, virtualizerStrength).apply()
    ...
    var wereSongFoldersAdded: Boolean
        get() = prefs.getBoolean(WERE_TRACK_FOLDERS_ADDED, false)
        set(wereTrackFoldersAdded) = prefs.edit()
            .putBoolean(WERE_TRACK_FOLDERS_ADDED, wereTrackFoldersAdded).apply()
